...
export default function Events() {
  ...
  const [toggleEventPublic] = useToggleEventPublicMutation();
  ...
  const handleTogglePublic = async (item) => {
    try {
      await toggleEventPublic({
        id: item.id,
        is_public: !item.is_public,
      }).unwrap();

      toast.success(
        !item.is_public ? "Tadbir faollashtirildi" : "Tadbir yashirildi",
      );
    } catch (err) {
    ...
    <div>
      ...
      <div className="mb-5 flex flex-wrap gap-3">
        ...
        <button
          type="button"
          onClick={() => {
            setUpcoming((prev) => (prev === undefined ? true : undefined));
          ...
          "
        >
          {upcoming ? "Keladigan tadbirlar" : "Barcha tadbirlar"}
        </button>
      </div>
      ...
      >
        <form onSubmit={submitHandler} className="space-y-6">
          ...
          <FormField
            ...
            onChange={(e) =>
              handleFieldChange("location_latin", e.target.value)
            }
          />
